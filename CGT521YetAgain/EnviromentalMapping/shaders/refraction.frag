...
uniform float refractRatio = 1.0;
...
void main(void) {
	...
	vec3 incomingRay = vec3(0.0);
	
	incomingRay = normalize(fPosition - cameraPosition);
	refractedRay = refract(incomingRay, normalize(fNormal), refractRatio);
	//Weird sampling
	refractedRay.xy = -refractedRay.xy;
	refraction = texture(skybox, refractedRay).rgb;
	
	color = mix(disneyShading(), refraction, mixFactor);
