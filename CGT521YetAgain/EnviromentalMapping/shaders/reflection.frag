...
void main(void) {
	...
	vec3 reflectRay = vec3(0.0);
	vec3 ray = vec3(0.0);
	
	ray = normalize(fPosition - cameraPosition);
	reflectRay = reflect(ray, normalize(fNormal));
	//Weird sampling
	reflectRay.xy = -reflectRay.xy;
	reflection = texture(skybox, reflectRay).rgb;
	
	color = mix(disneyShading(), reflection, mixFactor);
