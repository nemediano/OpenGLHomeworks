...
namespace image {
	ScreenGrabber::ScreenGrabber() : m_bytesPerPixel(3), m_frameNumber(0), m_clipNumber(0), m_height(0), m_width(0) {
	...
	void ScreenGrabber::grab(bool useFrontBuffer, const char* fileName) {
		...
		FIBITMAP* image = FreeImage_ConvertFromRawBits(screenBuffer, m_width, m_height, (3 * m_width), 24, 0x0000FF, 0xFF0000, 0x00FF00, false);
